...
Time-stamp: <2024-10-14 11:25:10 irumisugimori>
"""
...
def parse_solver(config: Any) -> Callable[[str], bool]:
    def parse(sval: str) -> bool:
        if sval not in ("clingo", "clingo-dl"):
            return False
        else:
            config.solver = sval
        return True
...
class HeulingoApp(Application):
    ...
    def register_options(self, options: ApplicationOptions):
        ...
        options.add(
            group, "solver",
            f"Use the ASP solver {{clingo|clingo-dl}} [{self.__config.solver}]",
            parse_solver(self.__config),
            argument="<arg>")
    ...
    def main(self, ctl: Control, files: Sequence[str]):
        ...
        if self.__config.solver == "clingo":
            for file in files:
                ctl.load(file)
            ...
            with ast.ProgramBuilder(ctl) as builder:
                ast.parse_files(files, lambda ast: thy.rewrite_ast(ast, builder.add))
            solver = SolverLNPS(ctl, self.__config, thy)
